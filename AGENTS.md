...
- **Scope on the send paths: the guard is held across proposal creation *and* the broadcast**, as upstream 3.10.0 held it. Upstream's MOB-1858 later proved outside the guard and, when the guard stayed busy, released the already-created transactions to the SDK's background resubmission (`broadcaster.releaseForResubmission`). This fork's SDK has no such call and its `TxResubmitter` skips transactions the app never submitted, so proving outside the guard would strand created transactions (inputs reserved, never broadcast) behind a "busy, try again" failure. Do not port `4783dae4`'s send-path half without that SDK API; its voting half (per-attempt guard in `VotingAPIClientLiveKey`) is in: the voting client takes the guard *per attempt* — `retryWithBackoff { withSubmission { … } }`, never the other way round, so the back-off sleeps happen with the guard free.
- **Acquisition timeout.** `withSubmission(timeout:)` / `TransactionGuard.acquire(timeout:)` bound how long a caller waits *for* the guard; the guarded body itself is never cut short. On expiry the acquirer throws `TransactionGuardBusyError` having never owned the guard — the deadline is enforced inside the actor, so it can only ever remove a waiter that is still queued and never abandons ownership a concurrent `release()` just handed over. Nothing in the app takes the timed form yet; `withSubmission(_:)` (unbounded) is the default everywhere, including both send paths.
