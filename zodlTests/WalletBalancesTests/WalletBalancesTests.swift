...
@Suite(.serialized) struct WalletBalancesTests {
    ...
    @MainActor @Test func transparentPoolBalanceIsDisplayOnlyAndIncludesAwaitingResolution() async {
        let store = makeIsolatedStore()

        await store.send(.balanceUpdated(fullPoolAccountBalance()))

        #expect(store.state.transparentBalance == Zatoshi(5))
        #expect(store.state.awaitingResolutionBalance == Zatoshi(1))
        #expect(store.state.transparentPoolBalance == Zatoshi(6))
        #expect(store.state.spendability == .something)
    }

    // The four displayed pool values must sum exactly to the wallet's total balance.
    @MainActor @Test func poolBalancesSumToTotalBalance() async {
        let store = makeIsolatedStore()

        await store.send(.balanceUpdated(fullPoolAccountBalance()))

        let poolSum = store.state.saplingPoolBalance
            + store.state.orchardPoolBalance
            + store.state.ironwoodPoolBalance
            + store.state.transparentPoolBalance
        #expect(poolSum == store.state.totalBalance)
    }
    ...
    @MainActor @Test func balanceUpdatedWithNilZeroesPoolBalances() async {
        ...
        #expect(store.state.transparentPoolBalance == .zero)
    }
    ...
    @MainActor
    private func makeIsolatedStore() -> TestStoreOf<WalletBalances> {
        withDependencies {
            $0.defaultInMemoryStorage = InMemoryStorage()
        } operation: {
            let store = TestStore(initialState: WalletBalances.State()) {
