...
@Suite(.serialized) struct SmartBannerTests {
    ...
    @MainActor @Test func replacingSyncErrorRefreshesLastKnownErrorMessage() async {
        await withDependencies {
            $0.defaultInMemoryStorage = InMemoryStorage()
        } operation: {
            let firstError = ZcashError.synchronizerNotPrepared
            let replacementError = ZcashError.compactBlockProcessorCritical
            let firstMessage = SyncStatusSnapshot.snapshotFor(state: .error(firstError)).message
            let replacementMessage = SyncStatusSnapshot.snapshotFor(state: .error(replacementError)).message
            #expect(firstMessage != replacementMessage)

            var synchronizerState = SynchronizerState.zero
            synchronizerState.syncStatus = .error(firstError)

            let store = TestStore(initialState: SmartBanner.State()) { SmartBanner() }
            store.exhaustivity = .off
            store.dependencies.mainQueue = .immediate

            await store.send(.synchronizerStateChanged(synchronizerState.redacted))
            #expect(store.state.lastKnownErrorMessage == firstMessage)

            synchronizerState.syncStatus = .error(replacementError)
            await store.send(.synchronizerStateChanged(synchronizerState.redacted))
            #expect(store.state.lastKnownErrorMessage == replacementMessage)
        }
    }
    ...
    @MainActor @Test func evaluatePriority8TreatsIronwoodOnlyBalanceAsNonZero() async {
        ...
        // concurrently running suite that nils or overwrites `selectedWalletAccount` can clobber
        // it between `withLock` and `.evaluatePriority8` and make this test pass vacuously.
        await withDependencies {
            $0.defaultInMemoryStorage = InMemoryStorage()
        } operation: {
