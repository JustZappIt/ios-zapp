...
//
//  On iOS this is not decoration. Zapp has no background lane: proving and broadcasting happen only
...
@Suite struct MigrationBannerRowTruthTests {
    ...
    @Test func bothWorkingStatesAskTheUserToStay() {
        let keepOpen = "Keep Zapp open on active phone screen"
    ...
    /// the user does by staying makes it prove. That row must NOT wear a "keep Zapp open" ask.
    ///
...
/// twelve "Preparing transaction…" spinners and a "Keep Zapp open on active phone screen" banner,
