...
//  someone "fixing" the Zapp product name in the button text back to another spelling.
//
...
@Suite struct IronwoodAnnouncementCopyTests {
    ...
    /// fragments still carry their spaces, so a future language cannot land the "choose?Our guide"
    ...
    // MARK: - Case 3: the primary button carries Zapp branding

    /// Upstream ships this button as "Go to Zodl"; this fork is Zapp, so every user-facing string
    /// that names the product must say "Zapp". If this test fails because an upstream port dragged
    /// the "Zodl" wording back into the catalogue, the fix is to rebrand that value — this test is
