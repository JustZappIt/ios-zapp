...
@Suite(.serialized, .timeLimit(.minutes(1))) @MainActor struct RootTransactionsMigrationRowsTests {
    ...
    @Test func fetchedTransactionsRetainsMigrationRowsAndPublishesPendingValue() async {
        // Fork addition: the state's `@Shared(.inMemory:)` values bind to whichever storage is
        // ambient WHEN THE STATE IS BUILT, so the pin has to wrap state and store together —
        // pinning only the store's scope leaves the reducer reading a different, empty storage.
        // Mirrors `RootTransactionsAccountSwitchTests`.
        await withDependencies {
            ...
            // Fork note: NOT `.finish()`. The reducer's state mutation is synchronous, whereas
            // `.finish()` awaits Root's follow-on effects — in this tree a `.noOp` refetch that
            // resolves to an EMPTY payload and overwrites both values, plus the fork-only pending
            // poller, which never completes. Assert the synchronous result instead.
