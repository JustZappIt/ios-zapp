...
//  handling actually lives in four upstream coordinators plus Zapp's unified-send Keystone path,
...
@Suite(.serialized) @MainActor struct KeystoneFirmwareCoordFlowTests {
    ...
    // MARK: - Unified-send swap metadata deferral

    /// Zapp's unified send flow is a fifth Keystone signing surface that does not exist upstream.
    /// A firmware refusal must not persist swap metadata from this primary send corridor either.
    @Test func blockedUnifiedSendSwapWritesNoSwapMetadata() async throws {
        try await withDependencies {
            $0.defaultInMemoryStorage = InMemoryStorage()
        } operation: {
            ...
            store.send(
                .path(
                    .element(
                        ...
                        action: .scan(.foundPCZT(signedPczt(stamp: (12, 4, 6))))
                    )
                )
            )
    ...
    private func makeUnifiedSendStore(
        ...
        var confirmState = SendConfirmation.State.initial
        confirmState.address = initialState.swapState.address
        confirmState.proposal = .testOnlyFakeProposal(totalFee: 10_000)
        confirmState.type = .swap
        initialState.path.append(.confirmWithKeystone(confirmState))
        initialState.path.append(.scan(Scan.State.initial))
