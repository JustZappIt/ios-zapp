...
private actor OfframpNearBridgeWorker {
                ...
                case .pending, .pendingDeposit, .processing:
                    guard pollingClock.now < deadline else {
                        ...
                        return AppleBridgeExecution(
                            succeeded: false,
                            terminal: false,
                            message: "The 1-Click bridge is still \(details.status.rawName) after \(waitedDescription). Resume to check again."
                        )
                    }
                }
            } catch {
    ...
    /// The wait as the message states it. Derived from `pollingTimeout` rather than written out,
    /// so retuning the deadline (or a test injecting a short one) cannot leave the copy claiming
    /// a duration nobody waited.
    private var waitedDescription: String {
        let seconds = pollingTimeout.components.seconds
