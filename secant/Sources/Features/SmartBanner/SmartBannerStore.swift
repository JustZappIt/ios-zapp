...
struct SmartBanner {
    ...
    private func syncStatusChangedEffect(state: inout State, latestState: RedactableSynchronizerState) -> Effect<Action> {
        ...
        if case .error = snapshot.syncStatus {
            ...
            // [#1948] Clear both on recovery, so a later identical error is seen as different and
            // re-arms. Upstream lacks this; it must survive their extraction of this helper.
            state.lastKnownErrorMessage = ""
