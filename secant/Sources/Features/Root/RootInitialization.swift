...
extension Root {
    ...
    enum InitializationAction {
        ...
        case clearZappWalletScopedState
    ...
    func initializationReduce() -> Reduce<Root.State, Root.Action> {
        Reduce { state, action in
            ...
            case .initialization(.retryStart):
                ...
                return .run { [state] send in
                    do {
                        ...
                        // Fork-only: restore the transaction subscriptions cancelled on
                        // backgrounding. Unconditional — they are independent of whether this pass
                        // started sync.
                ...
                do {
                    ...
                    return .run { send in
                        do {
                            ...
                            // Release the single-flight latch before re-entering initialization;
                            // otherwise the re-entry's own prepare is silently dropped.
            ...
            case .resetZashiSDKSucceeded:
                ...
                // both wipes are sequenced ahead of `.resetZashiKeychainRequest` so the reset chain
                // continues only once the worklet is down and the pokes have been withdrawn.
                return .merge(
