...
enum ChatMediaEncoder {
    ...
    static func encode(_ data: Data, supportedTypes: [UTType]) throws -> Encoded {
        ...
        let isGIF = supportedTypes.contains { $0.conforms(to: .gif) }
        let sourceURL = ChatMediaTemporaryFiles.makeURL(pathExtension: isGIF ? "gif" : "jpg")

        try data.write(to: sourceURL, options: .atomic)
        defer { ChatMediaTemporaryFiles.remove(sourceURL) }

        return try encode(fileURL: sourceURL, supportedTypes: supportedTypes)
    }
