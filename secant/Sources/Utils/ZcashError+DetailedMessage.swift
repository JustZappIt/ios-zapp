...
extension ZcashError {
    ...
    /// would not have helped. Any user-facing copy must therefore offer both remedies (update Zapp,
    ...
    /// `incompatibleServerMessage` below, and the server-switch action gated by
    /// `SmartBanner.State.lastKnownErrorIsIncompatibleServer`.
    var isIncompatibleServer: Bool {
        switch self {
        case .compactBlockProcessorWrongConsensusBranchId,
            .compactBlockProcessorNetworkMismatch,
            .compactBlockProcessorSaplingActivationMismatch,
            .compactBlockProcessorChainName,
    ...
    /// are exactly the branch IDs spelled out properly below. What someone relaying this to support
    /// needs is the server, both branch IDs in hex, and the error code, as discrete lines.
    ///
    /// This is the string the sync-error sheet displays and the Report button mails.
    var incompatibleServerMessage: String? {
        ...
        if case let .compactBlockProcessorWrongConsensusBranchId(expectedLocally, found) = self {
            lines.append(
                String(localizable: .syncMessageErrorBranchId(expectedLocally.hexDescription, found.hexDescription))
            )
        }
