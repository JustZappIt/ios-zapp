...
- [ZAPP-1] You can now send GIFs in chat, and GIFs you receive play instead of sitting there as a frozen first frame. Send them straight from the keyboard's GIF key, by pasting one, or from the photo picker — all three keep the original animation rather than flattening it to a still. Very large GIFs are refused with a message saying so instead of failing quietly.
