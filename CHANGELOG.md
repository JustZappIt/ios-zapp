...
- [ZAPP-1] You can now send GIFs in chat, and GIFs you receive play instead of sitting there as a frozen first frame. There is a GIF button next to the composer that opens a KLIPY-powered search, and you can also paste a GIF or pick one from your photos — all three keep the original animation rather than flattening it to a still. Very large GIFs are refused with a message saying so instead of failing quietly.
