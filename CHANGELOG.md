...
### Fixed
- [#1948] Syncing Error details now name the server Zapp is connected to and show both consensus branch IDs in hexadecimal (for example, `0x37a5165b`). The same diagnostics are included in support reports. For a network-rules mismatch where retrying cannot succeed, the sheet offers Switch server instead of Try again.
