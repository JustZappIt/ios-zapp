...
- [ZAPP-1] Bottom sheets no longer cut off their last button. Sheets sized themselves from a measurement taken before their side margins were applied, so any sheet whose text wrapped onto extra lines came up short and clipped whatever sat at the bottom — most visibly the OK button in the "How this works" sheets for paying a merchant and adding funds to Base, and the Add/Save button on the transaction note sheet.
